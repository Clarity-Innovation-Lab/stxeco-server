  ...
  **/
  ...
  fromHex: function (method, rawResponse, strResponse) {
    ...
    if (method === "get-mint-price") {
      ...
      method === "get-token-by-index" ||
      method === "get-token-by-hash" ||
      method === "get-edition-by-hash"
    ) {
      const clarityAsset = {};
      ...
      if (tokenData.saleData) {
        ...
        if (saleData.value) {
          const saleData = {};
          saleData.biddingEndTime =
            saleData.value.data["bidding-end-time"].value.toNumber();
          saleData.incrementPrice = this.fromMicroAmount(
            saleData.value.data["increment-stx"].value.toNumber()
          );
          saleData.reservePrice = this.fromMicroAmount(
            saleData.value.data["reserve-stx"].value.toNumber()
          );
          saleData.buyNowOrStartingPrice = this.fromMicroAmount(
            saleData.value.data["amount-stx"].value.toNumber()
          );
          saleData.saleType = saleData.value.data["sale-type"].value.toNumber();
          saleData.saleCycle =
            saleData.value.data["sale-cycle-index"].value.toNumber();
          clarityAsset.saleData = saleData;
        }
      }
      if (tokenData.tokenInfo) {
        clarityAsset.assetHash =
          tokenData.tokenInfo.value.data["asset-hash"].buffer.toString("hex");
        clarityAsset.edition =
          tokenData.tokenInfo.value.data.edition.value.toNumber();
        clarityAsset.seriesOriginal =
          tokenData.tokenInfo.value.data["series-original"].value.toNumber();
        clarityAsset.maxEditions =
          tokenData.tokenInfo.value.data["max-editions"].value.toNumber();
        clarityAsset.maxEditionCost =
          tokenData.tokenInfo.value.data["edition-cost"].value.toNumber();
        clarityAsset.date =
          tokenData.tokenInfo.value.data.date.value.toNumber();
      }
      ...
      return {
        ...
        incrementPrice: this.fromMicroAmount(
          res.value.data["increment-stx"].value.toNumber()
        ),
        reservePrice: this.fromMicroAmount(
          res.value.data["reserve-stx"].value.toNumber()
        ),
        buyNowOrStartingPrice: this.fromMicroAmount(
          res.value.data["amount-stx"].value.toNumber()
        ),
  ...
  resolveStacksTransactions: function (network, transactions) {
    transactions.forEach((transaction) => {
      if (transaction.amount)
        transaction.amount = this.fromMicroAmount(transaction.amount);
    });
    return transactions;
  },
  resolvePrincipalsTokens: function (network, tokens, sipTenTokens) {
    ...
    tokens.forEach((token) => {
      resolvedTokens.push(
        this.resolvePrincipalsToken(network, token, sipTenTokens)
      );
    });
    return resolvedTokens;
  },
  resolvePrincipalsGaiaToken: function (network, gaiaAsset, sipTenTokens) {
    gaiaAsset.contractAsset = this.resolvePrincipalsToken(
      network,
  ...
  resolvePrincipalsToken: function (network, token, sipTenTokens) {
    ...
    token.tokenInfo.editionCost = this.fromMicroAmount(
      token.tokenInfo.editionCost
    );
    ...
    if (token.listingInUstx && token.listingInUstx.price > 0) {
      token.listingInUstx.commission = this.convertAddressInt(
        network,
        token.listingInUstx.commission
      );
      token.listingInUstx.price = this.fromMicroAmount(
        token.listingInUstx.price
      );
      if (sipTenTokens && token.listingInUstx.token) {
        token.listingInUstx.token = this.convertAddressInt(
          network,
          token.listingInUstx.token
        );
        const sipTen = sipTenTokens.find(
          (o) => o.token === token.listingInUstx.token
        );
        if (sipTen) {
          token.listingInUstx.price = this.fromMicroAmount(
            token.listingInUstx.price,
    ...
    if (token.saleData) {
      token.saleData.buyNowOrStartingPrice = this.fromMicroAmount(
        token.saleData.buyNowOrStartingPrice
      );
      token.saleData.incrementPrice = this.fromMicroAmount(
        token.saleData.incrementPrice
      );
      token.saleData.reservePrice = this.fromMicroAmount(
        token.saleData.reservePrice
      );
    }
    if (token.beneficiaries) {
      ...
      token.beneficiaries.shares.forEach((share) => {
        token.beneficiaries.shares[idx].value =
          this.fromMicroAmount(share.value) / 100;
        token.beneficiaries.addresses[idx].valueHex = this.convertAddressInt(
          network,
          token.beneficiaries.addresses[idx].valueHex
        );
  ...
  resolvePrincipals: function (registry, network) {
    ...
    try {
      registry.administrator = this.convertAddressInt(
        network,
    ...
    if (registry.applications) {
      registry.applications.forEach((app) => {
        ...
        if (app.tokenContract) {
          app.tokenContract.administrator = this.convertAddressInt(
            network,
            app.tokenContract.administrator
          );
          app.tokenContract.mintPrice = this.fromMicroAmount(
            app.tokenContract.mintPrice
          );
        }
      });
    }
